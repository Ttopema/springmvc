...
@RestController //밑에 주석된 어노테이션 2개를 포함하는 어노테이션 //전부다 ResponseBody가 되는 것임.
//@Controller
...
public class JSONTestController {
	...
	// 컨트롤러 상단에 @ResponseBody를 선언했으므로 이 메소드 위에 @ResponseBody가 정의되어있는 것과 동일하다.
	// @ResponseBody는 이제 더이상 뷰를 응답하지 않고 response body에 스트링을 추가해서 response하겠다는 의미.
	...
	public BoardDTO responseObj() {
		// 리턴타입을 DTO로 명시하면 jackson-databind 라이브러리가 자동으로 json object로 변환해서 리턴함.
	...
	public List<BoardDTO> responseJsonArr() {
		// 리턴타입을 DTO로 명시하면 jackson-databind 라이브러리가 자동으로 json arry로 변환해서 리턴함.
